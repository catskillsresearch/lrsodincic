...
Adapted from the scott_models CARDB / LRSODInCIC PDF pipeline:
  ...
  1. lift the `# ...` title and Abstract/Scope paragraphs into \\title and `abstract`;
  2. pandoc the body, demoting headings by one so `##` Parts become \\section;
...
The generated `.tex` lives in this directory. `LRSODInCIC.tex` is git-ignored;
`LRSODInCIC.pdf` is the committed deliverable.
"""
...
GITHUB_URL = r"https://github.com/catskillsresearch/lrsodincic"
...
def build_appendix() -> str:
    return "\n".join(
        [
            ...
            "",
            r"Checked by \texttt{lake build} against Lean 4 with no Mathlib "
            r"dependency. The layer structures are those of Part III; the "
            r"closing sections are the Sierpi\'nski witness for D1--D4 and "
            r"its axiom audit.",
            "",
            listing_block(
                LEAN,
                r"Deep embedding of L/R/S/O/D and the Sierpi\'nski witness",
            ),
        ]
    )
