#!/usr/bin/env bash
# Build LRSODInCIC.tex and zip everything arXiv needs to compile it (pdfLaTeX).
#
# Template: scott2026/scripts/package_arxiv_submit.sh. This note has no mermaid
# figures and inlines Lean via \lstinputlisting{LRSODInCIC.lean}, so the zip is
...
TEX="LRSODInCIC.tex"
LEAN="LRSODInCIC.lean"
OUT_DIR="dist"
ZIP="${OUT_DIR}/arxiv_submit.zip"

if [[ "${1:-}" != "--skip-tex-build" ]]; then
  echo "==> Regenerating ${TEX} + LRSODInCIC.pdf"
...
readme = {
    ...
    "sources": [
        {"filename": "LRSODInCIC.tex", "usage": "toplevel"},
        {"filename": "LRSODInCIC.lean", "usage": "include"},
    ],
}
...
echo "replace). On arXiv Review Files: if LRSODInCIC.lean is marked for deletion, UNCHECK it."
