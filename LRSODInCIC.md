...
**Artifact.** `LRSODInCIC.lean` in this directory (standalone Lake package; `lake build`).
...
[`github.com/catskillsresearch/lrsodincic`](https://github.com/catskillsresearch/lrsodincic).
...
`PROVENANCE.md` records that migration.
Run `lake build` for the Lean embedding (no Mathlib).
Run `python3 build_pdf.py` to regenerate `LRSODInCIC.tex` and `LRSODInCIC.pdf` (title page, author, affiliation, and this GitHub path).
